...
namespace Kingthy.Test.Socks5.Server.Core
{
    ...
    public class TCPSocks5Connection
    {
        ...
        private void DoRequest(object state)
        {
            ...
            {
                if (this.DoShakeHands() & this.Server.RequireValidate& this.ValidateIdentity() & this.DoProtocolRequest())
                {
                    //建立代理
        ...
        private void CreateProxyBridge()
        {
            if (this.Client.Connected)
            {
                ...
                {
                    this.Server.WriteLineLog(string.Format("{0}------------------328", e.Message));
                    this.Close();
                }
            }
            else
        ...
        private void OnClientReceive(IAsyncResult result)
        {
            if (this.Server.IsStarting)
            {
                ...
                {
                    this.Server.WriteLineLog(string.Format("{0}------------------367", e.Message));
                    this.Close();
                }
            }
        }
        ...
        private void OnProxyReceive(IAsyncResult result)
        {
            if (this.Server.IsStarting)
            {
                ...
                {
                    this.Server.WriteLineLog(string.Format("{0}------------------402", e.Message));
                    this.Close();
                }
            }
        }
